// ============================================
// VPN CONFIG MANAGER - GATEWAY SERVER (ESM)
// Stabil, Anti-Crash, Full Features
// ============================================
...
// Setup global crypto untuk Node.js < 20
...
class GatewayServer {
  ...
  async handleApiRequest(req, res, parsedUrl) {
    try {
      ...
      // API: current-proxy
      if (parsedUrl.pathname === '/api/current-proxy') {
        const path = parsedUrl.query.path || '/';
        let pxip = '';
        
        // Coba dapatkan proxy dari path
        const proxyFromPath = await this.getProxyFromPath(path);
        if (proxyFromPath) pxip = proxyFromPath;
        
        res.writeHead(200, {
          'Content-Type': 'application/json',
          ...this.CORS_HEADER_OPTIONS
        });
        res.end(JSON.stringify({ path, proxy: pxip || null }));
        return;
      }
    } catch (error) {
      ...
      <body class="text-slate-300 min-h-screen flex flex-col justify-between selection:bg-blue-600 selection:text-white">
        ...
        <main class="max-w-7xl w-full mx-auto p-6 space-y-8 flex-grow">
          ...
          <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div class="bg-[#0d0e16] neon-border p-5 rounded-xl flex items-center justify-between">
              <div>
                <p class="text-xs text-slate-500 font-medium mb-1">CURRENT PATH</p>
                <p class="text-lg font-bold text-emerald-400" id="currentPath">/</p>
              </div>
              <i class="fa-solid fa-route text-emerald-900/50 text-2xl"></i>
            </div>
            <div class="bg-[#0d0e16] neon-border p-5 rounded-xl flex items-center justify-between">
              <div>
          ...
          <div class="bg-[#0d0e16] border border-slate-900 rounded-xl p-6 space-y-4">
            <div class="flex items-center gap-2 border-b border-slate-900 pb-3">
              ...
              <h2 class="text-md font-bold tracking-wide text-white">CURRENT ROUTING TARGET</h2>
            </div>
            <div class="flex flex-wrap gap-4 items-center">
              <div class="bg-[#10121d] border border-slate-900/60 px-4 py-3 rounded-lg">
                <span class="text-xs text-slate-500">PROXY:</span>
                <span class="text-sm font-mono text-emerald-400 font-bold ml-2" id="currentProxy">-</span>
              </div>
              <button onclick="copyCurrentProxy()" class="bg-blue-600/10 border border-blue-500/20 text-blue-400 hover:bg-blue-600/30 px-4 py-2 rounded-lg text-xs font-bold transition-all flex items-center gap-2">
                <i class="fa-regular fa-copy"></i> COPY PROXY
              </button>
            </div>
          </div>
        ...
        <script>
          ...
          // Update current path & proxy
          document.getElementById('currentPath').innerText = window.location.pathname || '/';
          fetch('/api/current-proxy?path=' + encodeURIComponent(window.location.pathname))
            .then(r => r.json())
            .then(d => { document.getElementById('currentProxy').innerText = d.proxy || 'No proxy'; })
            .catch(() => { document.getElementById('currentProxy').innerText = 'Error'; });
  ...
  async handleHttpRequest(req, res) {
    ...
    if (parsedUrl.pathname === '/') {
      ...
      res.end(this.getDashboardHTML(req));
      return;
    }
  ...
  async getProxyFromPath(pathname) {
    ...
    try {
      ...
      if (proxyListMatch) {
        ...
        return kvPrx[prxKey][Math.floor(Math.random() * kvPrx[prxKey].length)];
      }

      // Format /ALL
      const allMatch = pathname.match(/^\/ALL(\d+)?$/i);
      if (allMatch) {
      ...
      if (putarMatch) {
        ...
        const prxKey = countries[Math.floor(Math.random() * countries.length)];
        return kvPrx[prxKey][Math.floor(Math.random() * kvPrx[prxKey].length)];
      }

      // Format /REGION
      const regionMatch = pathname.match(/^\/([A-Z]+)(\d+)?$/i);
      if (regionMatch) {
      ...
      // Format /CC (Country Code)
      const countryMatch = pathname.match(/^\/([A-Z]{2})(\d+)?$/);
      if (countryMatch) {
        const countryCode = countryMatch[1].toUpperCase();
        if (!kvPrx[countryCode] || kvPrx[countryCode].length === 0) return null;
        return kvPrx[countryCode][Math.floor(Math.random() * kvPrx[countryCode].length)];
      }

      // Format /ip:port
      const ipPortMatch = pathname.match(/^\/(.+[:=-]\d+)$/);
      if (ipPortMatch) {
        return ipPortMatch[1].replace(/[=:-]/, ":");
      }

    } catch (err) {
      console.error('Error in getProxyFromPath:', err);
    }
  ...
  async handleWebSocketConnection(ws, request) {
    try {
      ...
      // Dapatkan proxy dari path
      const proxyFromPath = await this.getProxyFromPath(path);
      if (proxyFromPath) {
        this.prxIP = proxyFromPath;
        console.log(`Routed via path ${path} -> ${this.prxIP}`);
        await this.websocketHandler(ws);
        return;
      }

      // Format /ip:port atau /ip=port atau /ip-port langsung
...
try {
  // Dynamic import for dotenv (optional)
  import('dotenv').then(m => m.config()).catch(() => {});
} catch (e) {}
server.start();
